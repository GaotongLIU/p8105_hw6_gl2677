...
* The distribution of `r_squared` is nearly normal distribution with slight left skewness. The peak density is obtained when `r_squared` is approximately 0.91, and the range of `r_squared` is from 0.8697 to 0.9431.

* The distribution of `log(beta_0*beta_1)` is nearly normal distribution and the peak density is obtained when `log(beta_0*beta_1)` is approximately 2.01, and the range of `r_squared` is from 1.918  to 2.106.
